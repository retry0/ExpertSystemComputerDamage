...
$("div.btn-input").live('click', function(){
    ...
    btn.addClass('active btn-primary');
		...
		//tampilkan pertanyaan pertama
			$sqlp = "select * from diagnosa_motherboard where mulai='Y'";
			$rs=mysql_query($sqlp);
		...
		echo "<form>";
			echo "<legend>Memeriksa <small> Mari mulai Memeriksa </small></legend>";
			echo "<center>";
		...
		//tampilkan pertanyaan pertama
			...
			$sqlp = "select * from diagnosa_motherboard where id=$idsolusi";
		...
		echo "<form>";
			...
			if($data['selesai']!="Y"){
				...
				echo "<input type='submit' class='btn btn-primary btn-block btn-large' value='Lanjut ' >";
                echo "<a class='btn  btn-block btn-large	' href='Solusi_motherboard.php?idpertanyaan=1'>Solusi </a>"; 		
			}else{
			//jika ingin menambah pertanyaan
			}
			echo "</form>";		
			echo '</br><div class=" well">
			<center><p>Lingga Adi Pratama</p></center> </div>';
		}
		?>
        <center><img src="Mobo.jpg" width="728" height="400" />
        <a href='''' class='btn  btn-block btn-large ' /> <H1>Kembali Memeriksa yang lainnya</H1> </a>
	</div>
</div>
